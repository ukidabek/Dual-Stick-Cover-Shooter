...
namespace Interactions
{
    ...
    public class OnTrggerInteractionUser : MonoBehaviour, IUse
    {
        ...
        [SerializeField] private IInteraction _interaction = null;
        ...
        private void Update()
        {
            if (interactions.Count > 0)
            {
                float distance = Vector3.Distance(transform.root.transform.position, interactions[0].Position);
                var closestInteraction = interactions[0];

                for (int i = 1; i < interactions.Count; i++)
                {
                    float tmpDistance = Vector3.Distance(transform.root.transform.position, interactions[i].Position);
                ...
                {
                    _targetDirection = _interaction.Position - transform.root.transform.position;
